...
void Application::input()
{
	...
	auto checkInput = [&]() {
		...
		if (m_input.velocity1 < m_input.velocity2)
		{
	...
	if (elapsed > Settings::TIME_TO_COLLISION)
	{
		...
		ImGui::Text("Ball 1"); ImGui::SameLine(m_mainWindowBounds.width - 92); ImGui::Text("%.2f", m_ball1.velocity);
		ImGui::Text("Ball 2"); ImGui::SameLine(m_mainWindowBounds.width - 92); ImGui::Text("%.2f", m_ball2.velocity);
		ImGui::End();
	}
}
...
void Application::update(float deltatime)
{
	...
	if (m_ball1.resolveCollision(m_ball2))
	{
		...
		float b = -2 * (m_input.mass1*m_input.velocity1 + m_input.mass2*m_input.velocity2)*m_input.mass2 / (2 * m_input.mass1);
		float c = std::pow((m_input.mass1*m_input.velocity1 + m_input.mass2*m_input.velocity2), 2) / (2 * m_input.mass1)
			- 0.5f*m_input.mass1*std::pow(m_input.velocity1, 2)	- 0.5f*m_input.mass2*std::pow(m_input.velocity2, 2);
...
{
	float accel1 = m_input.velocity1 / Settings::ACCEL_TIME;
	float accel2 = m_input.velocity2 / Settings::ACCEL_TIME;

	float deltaX1 = 0.5f * accel1 * std::pow(Settings::ACCEL_TIME, 2) + m_input.velocity1 * Settings::COASTING_TIME;
	float deltaX2 = 0.5f * accel2 * std::pow(Settings::ACCEL_TIME, 2) + m_input.velocity2 * Settings::COASTING_TIME;

	m_ball1.initPhysics({ m_windowDimensions.x / 2 - Ball::RADIUS - deltaX1,(float)m_windowDimensions.y / 2 }, accel1, m_input.mass1);
	m_ball2.initPhysics({ m_windowDimensions.x / 2 + Ball::RADIUS - deltaX2,(float)m_windowDimensions.y / 2 }, accel2, m_input.mass2);
